...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
GPIO_TypeDef* ROW_PORT[4] = {R_PORT, R_PORT, R_PORT, R_PORT};
uint16_t ROW_PIN[4] = {R1_PIN, R2_PIN, R3_PIN, R4_PIN};

GPIO_TypeDef* COL_PORT[4] = {C_PORT, C_PORT, C_PORT, C_PORT};
...
uint8_t str[16];
uint8_t sNum[5];
// Định nghĩa số lượng thẻ được phép
#define NUM_ALLOWED_CARDS 20

// Danh sách các UID hợp lệ (Master Database)
const uint8_t Allowed_UIDs[NUM_ALLOWED_CARDS][5] = {
    {44, 245, 250, 3, 32},   // Thẻ 1
    {227, 26, 186, 27, 88},   // Thẻ 2
	{186, 233, 43, 26, 98},
    {52, 86, 74, 90, 114},
    {52,208 ,134 ,90 ,56 }
};
...
uint8_t Check_Card_Access(uint8_t *inputUID) {
    ...
    // Duyệt qua từng thẻ trong danh sách cho phép
    for (i = 0; i < NUM_ALLOWED_CARDS; i++) {
        match_count = 0;

        // So sánh từng byte của thẻ (5 bytes)
        for (j = 0; j < 5; j++) {
            if (inputUID[j] == Allowed_UIDs[i][j]) {
                match_count++;
            } else {
                break; // Nếu lệch 1 byte thì bỏ qua thẻ này ngay
            }
        }

        // Nếu khớp đủ 5 byte -> Thẻ hợp lệ
        if (match_count == 5) {
            return 1; // Hợp lệ (True)
        }
    }

    return 0; // Không tìm thấy thẻ nào khớp (False)
}
